...
/** Default "home" surface for a Files tab with no file open: a search pill over
 *  reused sidebar sections (Actions + Recent Files). The Actions rows are static
 *  filler; Recent Files reopen the file in place (matching the sidebar tree) and
 *  can be dragged out into a tab anywhere, like the tree rows. */
export function FilesHome({ tab, tileId }: { tab: Tab; tileId: string }) {
  const scopeId = useActiveScopeId();
  const recents = getRecentFiles(scopeId);
